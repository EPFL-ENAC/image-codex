...
async def create_composition(
    composition: Composition, mimetype: str = Query("application/pdf")
) -> ApiFile:
    """
    Create an image composition and return it in the given type
    """
    composedImage = Image.new(
        "RGB", (composition.width, composition.height), composition.background_color
    )
    ...
    return ApiFile(name=composition.name, type=mimetype, base64=data)
